...
```bash
# npm
npm install result_option

# pnpm
pnpm add result_option

# yarn
yarn add result_option

# bun
bun add result_option
```
...
Option.None<number>().map_or(0, (x) => x * 2);             // 0

Option.Some(10).map_or_else(() => 0, (x) => x * 2);        // 20
Option.None<number>().map_or_else(() => 0, (x) => x * 2);  // 0
```
...
Option.None<number>().and_then((x) => Option.Some(x * 2)); // None

Option.Some(2).or(Option.None());                           // Some(2)
Option.None<number>().or(Option.Some(100));                 // Some(100)

Option.Some(2).or_else(() => Option.Some(100));             // Some(2)
Option.None<number>().or_else(() => Option.Some(100));      // Some(100)
```
...
Result.Err<number, string>("err").map((x) => x * 2);        // Err("err")

Result.Ok(10).map_or(0, (x) => x * 2);                     // 20
Result.Err("err").map_or(0, (x) => x * 2);                 // 0

Result.Ok(10).map_or_else((e) => 0, (x) => x * 2);         // 20
Result.Err("err").map_or_else((e) => 0, (x) => x * 2);     // 0

Result.Err("oops").map_err((e) => e.length);                // Err(4)
Result.Ok<number, string>(10).map_err((e) => e.length);     // Ok(10)
```
...
Result.Err<number, string>("err").and_then((x) => Result.Ok(x * 2)); // Err("err")

Result.Ok(10).or(Result.Err("late"));                       // Ok(10)
Result.Err("err").or(Result.Ok(0));                         // Ok(0)

Result.Ok(10).or_else(() => Result.Ok(0));                  // Ok(10)
Result.Err("err").or_else(() => Result.Ok(0));              // Ok(0)
```
...
Result.Err("oops").err();     // Option.Some("oops")
